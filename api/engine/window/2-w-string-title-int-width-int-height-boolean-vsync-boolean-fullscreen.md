...
**Visibility:** `public`  
**Signature:** `public Windo w(String title, int width, int height, boolean vsync, boolean fullscreen)`
...
| `fullscreen` | `boolean` | Caller-supplied input; verify range, units, lifecycle, and nullability. |
...
- `boolean fullscreen`: validate type, range, coordinate space, ownership, and nullability before calling.
...
Windo result = instance.w(title, width, height, vsync, fullscreen);
```
