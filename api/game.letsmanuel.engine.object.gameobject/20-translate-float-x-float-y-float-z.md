...
**Visibility:** `public`  
**Signature:** `public GameObject translate(float x, float y, float z)`

## What it does

This function belongs to `game.letsmanuel.engine.object.GameObject` and performs the operation represented by `translate`. Use it according to the lifecycle and ownership rules of the declaring type.
...
GameObject result = instance.translate(x, y, z);
```
...
When modifying code that calls `translate`, preserve argument order and types, search existing call sites first, keep lifecycle ownership explicit, and update this page if behavior changes.
