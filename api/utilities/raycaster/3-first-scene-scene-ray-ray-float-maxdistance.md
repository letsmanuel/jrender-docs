...
**Signature:** `public static RayHit first(Scene scene, Ray ray, float maxDistance)`

## What it does

This function belongs to `game.letsmanuel.engine.ray.Raycaster` and performs the operation represented by `first`. Use it according to the lifecycle and ownership rules of the declaring type.
...
Returns a value of type `RayHit`.

## Code sample

```java
// Obtain or construct the owning instance before this call.
RayHit result = instance.first(scene, ray, maxDistance);
```
...
When modifying code that calls `first`, preserve argument order and types, search existing call sites first, keep lifecycle ownership explicit, and update this page if behavior changes.
