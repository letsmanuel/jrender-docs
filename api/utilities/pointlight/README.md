# PointLight

**Package:** `game.letsmanuel.engine.light`  
**Source path:** `engine/src/main/java/game/letsmanuel/engine/light/PointLight.java`

## Purpose

PointLight is part of the Letsmanuel engine API. Each function page documents its signature, arguments, return behavior, sample usage, common issues, reusable template, and agent instructions.

## Functions

- [position](./1-position.md)
- [color](./2-color.md)
- [intensity](./3-intensity.md)
- [range](./4-range.md)
- [setPosition](./5-setposition-float-x-float-y-float-z.md)
- [setPosition](./6-setposition-vector3f-p.md)
- [setColor](./7-setcolor-float-r-float-g-float-b.md)
- [setColor](./8-setcolor-vector3f-c.md)
- [setIntensity](./9-setintensity-float-v.md)
- [setRange](./10-setrange-float-v.md)



## Best practices

- Keep IDs, dimensions, speeds, durations, colors, and ranges in named constants or configuration objects.
- Initialize the declaring type from the engine lifecycle stage described in the relevant theory page.
- Retain references to objects that must be updated; avoid repeated construction inside the frame loop.
- Validate external values before passing them to native rendering, audio, window, or persistence APIs.
- Preserve ownership: the subsystem that creates a native resource is responsible for releasing it.
