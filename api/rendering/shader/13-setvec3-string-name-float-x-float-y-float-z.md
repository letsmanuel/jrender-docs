...
**Visibility:** `public`  
**Signature:** `public void setVec3(String name, float x, float y, float z)`

## What it does

This function belongs to `game.letsmanuel.engine.render.Shader` and performs the operation represented by `setVec3`. Use it according to the lifecycle and ownership rules of the declaring type.
...
When modifying code that calls `setVec3`, preserve argument order and types, search existing call sites first, keep lifecycle ownership explicit, and update this page if behavior changes.
