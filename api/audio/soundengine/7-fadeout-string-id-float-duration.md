...
**Visibility:** `public`  
**Signature:** `public void fadeOut(String id, float duration)`

## What it does

This function belongs to `game.letsmanuel.engine.SoundEngine` and performs the operation represented by `fadeOut`. Use it according to the lifecycle and ownership rules of the declaring type.
...
When modifying code that calls `fadeOut`, preserve argument order and types, search existing call sites first, keep lifecycle ownership explicit, and update this page if behavior changes.
