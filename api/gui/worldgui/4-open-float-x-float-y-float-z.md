...
**Visibility:** `public`  
**Signature:** `public WorldGui open(float x, float y, float z)`

## What it does

This function belongs to `game.letsmanuel.engine.gui.WorldGui` and performs the operation represented by `open`. Use it according to the lifecycle and ownership rules of the declaring type.
...
WorldGui result = instance.open(x, y, z);
```
...
When modifying code that calls `open`, preserve argument order and types, search existing call sites first, keep lifecycle ownership explicit, and update this page if behavior changes.
