...
**Visibility:** `public`  
**Signature:** `public GuiText setBackground(float r, float g, float b, float a)`

## What it does

This function belongs to `game.letsmanuel.engine.gui.GuiText` and performs the operation represented by `setBackground`. Use it according to the lifecycle and ownership rules of the declaring type.
...
GuiText result = instance.setBackground(r, g, b, a);
```
...
When modifying code that calls `setBackground`, preserve argument order and types, search existing call sites first, keep lifecycle ownership explicit, and update this page if behavior changes.
